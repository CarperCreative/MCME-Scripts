...
public class EnchantmentAddAction extends EntityTargetedAction {

    public EnchantmentAddAction(EntityEventTarget target, Set<ItemFilter> itemFilters, Set<WrappedEnchantment> enchantments, Set<LootTableChoice<WrappedEnchantment>> enchantmentChoices, int quantity, int duration) {
        super(target, (entity, context) -> {
            ...
            if (calculatedQuantity >= 0) {
                for (ItemStack applyItem : applyItems) {
                    ...
                    applyEnchantments.addAll(lootTable.select());
